...
##### `time` - используется для управления прогрессом трека
```json
{
  "request": "time",
  "message": 15,
...
```
`message` - количество секунд на которое нужно перемотать прогресс трека вперёд/назад

`how` - направление изменения, может равняться `0` или `1`. Скорее всего позже будет изменено на `-1` и `1`.
...
&nbsp;
##### `currentTime` - возвращает прогресс текущего трека в виде секунд.
...
&nbsp;
##### `endTime` - возвращает длину текущего трека в виде секунд.
